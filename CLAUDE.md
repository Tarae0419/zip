...
- `lib/db/schema.ts` — Drizzle schema, the source of truth for the data model. Twelve tables: `courses`, `course_department_tracks`, `field_tags`, `course_field_tags`, `industry_tags`, `course_industry_tags`, `course_embeddings`, `reviews`, `summaries`, `users`, `email_verifications`, `curricula`. `industry_tags.embedding` and `course_embeddings.embedding` are pgvector columns (`vector(1536)`, extension enabled on the Neon project). `users.role`/`users.status` and `curricula.data_status` (migration `0006`, Sprint 9) back the admin surface — see Admin bullet above.
