...
- `lib/db/scripts/import-courses.ts` — one-off/re-runnable importer for the four official course-catalog xlsx files, all under the `course/` folder (not repo root — moved there 2026-07-30 when the 교양 files were added): `2026_1학기_학부전공_개설교과목_목록.xlsx`, `2026_2학기_학부전공_개설교과목_목록.xlsx`, `2026_1학기_교양.xlsx`, `2026_2학기_교양.xlsx`. Same column layout across all four. Inserts are `onConflictDoNothing` on `(code, section, semester)`, so re-running is safe.
...
- 교양(gen-ed) courses are present since 2026-07-30 (`course/2026_1·2학기_교양.xlsx`, same column layout as the major-course files, `requirementType` = `"교양"`). Before that, only 학부전공(major) courses existed and `교양영역구분`/`requirementType` "교양" was never actually populated.
