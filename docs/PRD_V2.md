...
> 현재 구현 상태: TMAP 서버 경로 계산, Vector 지도, 앱 내부 경로선과 구간별 `도보 n분`, 한 수업인 날의
> 건물 지도·핀, 부분 실패, 요청 취소와 23시간 프로세스 캐시를 구현했다. 출입구 수동 검수와 운영 쿼터
> 계측은 남아 있다.

**사용자 스토리**

- 학생으로서, 다른 지도 앱으로 이동하지 않고 시간표 화면에서 다음 수업까지의 실제 보행 경로를 보고 싶다.
- 학생으로서, 각 이동 구간에서 그 순간 필요한 도보 시간만 지도 위에서 바로 확인하고 싶다.
- 학생으로서, 하루에 수업이 하나뿐이어도 해당 수업 건물의 위치를 지도에서 확인하고 싶다.
...
5. 하루 동선은 연속 수업 구간을 각각 계산하고 지도에서 구간 순서를 구분한다. 각 실제 경로의 중간 지점에
   `도보 n분`을 표시하며, 하루 전체 거리·시간 합계는 사용자 화면에 표시하지 않는다.
...
14. 선택한 요일에 수업이 하나뿐이면 경로 API를 호출하지 않고 해당 건물 핀과 수업 정보를 TMAP 지도에 표시한다.
...
- 여러 구간 중 하나만 실패하면 성공 구간은 유지하고 실패 구간만 `추정` 또는 별도 안내로 구분한다.
...
- [ ] 각 경로 위 `도보 n분`이 해당 구간 응답과 일치하고, 하루 총합 없이 부분 실패 상태가 구분된다.
- [ ] 수업이 하나인 날에도 TMAP 지도와 해당 건물 핀이 표시되며 경로 REST API는 호출하지 않는다.
