...
| F5 시간표·이동동선 | TMAP 앱 내부 도보 경로를 최우선으로 제공하고 NAVER 지도 앱 연결은 폴백으로 유지. F10 수업 허브 및 F15 후보 비교의 현재 학기 실행 화면으로 사용 |
...
잘못 판단할 수 있다. 현재 구현된 NAVER 지도 앱 URL Scheme은 실제 보행 길찾기를 제공하지만 사용자가 수강길잡이
화면을 벗어나야 하므로, 목표 경험인 앱 내부 경로 확인을 충족하지 못한다.

> 현재 구현 상태: 지도 핀, 직선 경로 제거, NAVER 지도 앱 도보 길찾기 폴백까지 구현되어 있다. TMAP 경로 계산,
> 앱 내부 경로선·거리·시간 표시는 아직 구현되지 않았다.

**사용자 스토리**

- 학생으로서, 다른 지도 앱으로 이동하지 않고 시간표 화면에서 다음 수업까지의 실제 보행 경로를 보고 싶다.
- 학생으로서, 각 이동 구간과 하루 전체의 도보 거리·예상 시간을 확인하고 싶다.
- 학생으로서, 경로를 계산할 수 없을 때 추정값과 외부 길찾기 폴백을 명확히 구분하고 싶다.
...
8. TMAP 지도 또는 보행 경로가 실패하면 실제 경로선을 만들지 않는다. 직선거리 추정값에는 `추정`을 표시하고,
   모바일에서는 기존 NAVER 지도 앱 `nmap://route/walk` 연결을 폴백으로 제공한다.
9. TMAP 경로를 NAVER 또는 Kakao 바탕지도에 겹쳐 표시하는 것은 교차 사업자 이용 허용을 서면 확인하기 전까지
   사용하지 않는다. 실제 보행 경로 화면은 TMAP 지도와 TMAP 경로를 함께 사용한다.
...
- NAVER 폴백은 지도 앱이 없는 iOS에서 실패할 수 있으므로 앱 설치 안내 또는 App Store 이동을 실제 기기에서 검증한다.
...
- [ ] TMAP 실패 시 직선거리 값은 `추정`으로 표시되고 모바일 NAVER 도보 길찾기 폴백이 동작한다.
...
| 지도·보행 경로 | NAVER·Kakao 지도 핀, NAVER 앱 URL Scheme | TMAP Vector JS와 보행 REST를 앱 내부 기본값으로 전환. NAVER 앱 연결은 폴백 |
