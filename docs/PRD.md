...
> 2026-07-30 신규 추가 — 아직 미구현(Draft) 상태다. 다른 장과 달리 이 13장은 실제 구현이 진행되는 동안에도
> (다른 F1~F4 장과 같이 "고정 스펙"으로 얼리지 않고) `docs/SPRINT_PLAN.md` 진행 상황에 맞춰 함께 갱신한다 —
> 관리자 화면은 운영 도구 성격이라 F1~F4보다 요구사항이 구현 중 바뀔 가능성이 높기 때문이다.
