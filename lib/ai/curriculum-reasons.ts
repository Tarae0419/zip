 ...
 */
export async function writeElectiveReasons(
  items: ElectiveReasonInput[],
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `너는 대학 커리큘럼 추천 이유를 한 문장으로 자연스럽게 써주는 도우미야.
        각 과목마다 왜 이 과목을 추천하는지 한국어 한 문장으로 써. 관심분야와의 연관성을 반영하고,
        본인 전공이 아니면 "타 전공 과목이라 수강 가능 여부를 별도로 확인해야 한다"는 점도 짧게 덧붙여.
        없는 사실을 지어내지 말고 과장하지 마. 반드시 다음 JSON 형식으로만 응답해
        : {"reasons": [{"code": "학수번호", "reason": "..."}]} 모든 입력 과목에 대해 결과를 하나씩 포함해.`,
      },
      {
        role: 'user',
