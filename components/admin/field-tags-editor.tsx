...
import { adminAddFieldTag, adminRemoveFieldTag } from "@/lib/actions/admin/courses"

export function FieldTagsEditor({ courseId, tags }: { courseId: string; tags: { id: string; name: string }[] }) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = await adminAddFieldTag(courseId, name)
  ...
  function handleRemove(tagId: string) {
    startTransition(async () => {
      await adminRemoveFieldTag(courseId, tagId)
    })
  }
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        ...
        {tags.length === 0 ? <span className="text-xs text-muted-foreground">아직 분야 태그가 없어요.</span> : null}
      </div>
      <div className="mt-2 flex items-center gap-2">
        <input
          ...
          placeholder="분야 태그명 (기존 이름과 같으면 그 태그에 연결)"
